...
class Encoder:
    ...
    def decode(self, bpe_idx):
        ...
        chars = []
        for c in tokens_flat:
            if not c in self.byte_decoder:
                break
            chars.append(self.byte_decoder[c])

        tokens_bytes = bytearray(chars)
...
def get_encoder(vocab_file):
    ...
    lines = open(vocab_file).read().split('\n')
    encoder = {}
    bpe_merges = []
    it = iter(range(len(lines)))
    vocab_size = int(lines[next(it)])
    for i in range(vocab_size):
      encoder[lines[next(it)]] = i+1
    encoder['\n'] = 0
    
    for i in range(int(lines[next(it)])):
      j = next(it)
      a, b = lines[j].split()
      bpe_merges.append((a, b))
...
class BPETokenizer:
    ...
    def __init__(self, vocab_file='./vocab.bpe'):
        self.encoder = get_encoder(vocab_file)
...
if __name__ == '__main__':
    ...
    text = "Hi this is very interesting. Cooool stuff!"
    e = get_encoder(vocab_file='./vocab.bpe')
